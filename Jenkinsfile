    ...
    stages{
        stage("Clone"){
            steps{
                ...
                // checkout scm
                git 'https://github.com/demenzio/docker.easyepg.git'
            }
            post{
                success{
                    echo "====> Clone - Success"
        ...
        stage("Test"){
            steps{
                sh (label: "Starting Test Container of ${registry}:${newVersion}", script: "docker run -it -d --name test-build${env.BUILD_ID}-easyepgPlus ${registry}:${newVersion} /bin/sh")
                sh (label: "Execute Test Command in ${registry}:${newVersion}", script: "docker exec test-build${env.BUILD_ID}-easyepgPlus bash -c 'echo ContainerisRunning'")
                //VERSION CHECK MISSING + ee script basic cmds
                sh (label: "Stopping Test Container of ${registry}:${newVersion}", script: "docker stop test-build${env.BUILD_ID}-easyepgPlus")
            }            
            post{
                cleanup{
                    sh (label: "Removing Test Containers of ${registry}", script: "docker rm -f test-build${env.BUILD_ID}-easyepgPlus")                 
                }
                success{
                    echo "====> Test - Passed"
                }
